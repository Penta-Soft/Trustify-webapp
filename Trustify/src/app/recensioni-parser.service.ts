...
export class RecensioniParserService {
  ...
  async retriveHomePageReviews(from: number, to: number, address: string): Promise<Recensione[]> {
    return this.web3.GetNCompanyReview(from, to, address).then((reviews) => {
      let homePageReviews: Recensione[] = [];
      for (let currentReview = 0; currentReview < reviews[this.REVIEW_INDEX].length; currentReview++) {
  ...
  async retrivePersonalAreaReviews(from: number, to: number): Promise<Recensione[]> {
    return this.web3.GetNMyReview(from, to).then((reviews) => {
